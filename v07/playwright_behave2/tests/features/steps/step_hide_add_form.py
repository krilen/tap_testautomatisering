...
def step_then__form_for_adding_player_name_is_shown(context):
    ...
    expect(context.start_page.button_to_add_player()).to_be_visible(timeout=200) # Always visible in some way
...
def step_then__form_for_adding_player_name_is_not_shown(context):
    ...
    expect(context.start_page.button_to_add_player()).to_be_visible(timeout=200) # Always visible in some way
    expect(context.start_page.input_to_add_player()).not_to_be_visible()
